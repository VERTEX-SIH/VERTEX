...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Check URL parameters for OAuth errors returning from Supabase
    if (typeof window !== 'undefined') {
      ...
      if (errorDesc) {
        ...
        if (decoded.includes('provider is not enabled') || decoded.includes('Unsupported provider')) {
          setError('Google Sign-In is disabled on your Supabase project. Enable Google in Supabase Dashboard -> Authentication -> Providers.');
        } else {
          setError(decoded);
        }
      }
        ...
        <div className="w-full max-w-[440px] bg-surface-container-low/95 border border-outline-variant shadow-2xl backdrop-blur-md relative z-10 overflow-hidden flex flex-col my-auto">
          ...
          <div className="flex border-b border-outline-variant bg-surface shrink-0">
            <button
              ...
              className={`flex-1 py-2.5 flex items-center justify-center gap-2 transition-colors border-b-2 font-mono-label text-[10px] tracking-widest uppercase cursor-pointer ${
                mode === 'LOGIN'
                  ? 'border-primary bg-surface-container-highest text-on-surface font-bold'
                  : 'border-transparent text-secondary hover:bg-surface-container-high hover:text-primary'
              }`}
            >
          ...
          <div className="p-6 overflow-y-auto space-y-5">
            ...
            <div>
              ...
              >
                ...
                <span className="group-hover:text-primary transition-colors uppercase font-bold">
                  {googleLoading ? 'CONNECTING GMAIL...' : 'CONTINUE WITH GMAIL / GOOGLE'}
                </span>
              </button>
            </div>
            ...
            {error && (
              <div className="bg-error-container text-on-error-container p-3 border border-error text-[11px] font-mono leading-relaxed flex items-start gap-2">
                ...
                <div>
                  <div className="font-bold uppercase tracking-wider">AUTHENTICATION NOTICE</div>
